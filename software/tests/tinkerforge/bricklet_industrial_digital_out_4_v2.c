...
typedef void (*WiegandDone_CallbackFunction)(void *user_data);
...
} ATTRIBUTE_PACKED WriteWiegandDataLowLevel_Request;

typedef struct {
	PacketHeader header;
} ATTRIBUTE_PACKED GetWiegandState_Request;

typedef struct {
	PacketHeader header;
	uint8_t wiegand_state;
} ATTRIBUTE_PACKED GetWiegandState_Response;

typedef struct {
	PacketHeader header;
} ATTRIBUTE_PACKED WiegandDone_Callback;
...
static void industrial_digital_out_4_v2_callback_wrapper_wiegand_done(DevicePrivate *device_p, Packet *packet) {
	WiegandDone_CallbackFunction callback_function;
	void *user_data;
	WiegandDone_Callback *callback;

	if (packet->header.length != sizeof(WiegandDone_Callback)) {
		return; // silently ignoring callback with wrong length
	}

	callback_function = (WiegandDone_CallbackFunction)device_p->registered_callbacks[DEVICE_NUM_FUNCTION_IDS + INDUSTRIAL_DIGITAL_OUT_4_V2_CALLBACK_WIEGAND_DONE];
	user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + INDUSTRIAL_DIGITAL_OUT_4_V2_CALLBACK_WIEGAND_DONE];
	callback = (WiegandDone_Callback *)packet;
	(void)callback; // avoid unused variable warning
...
void industrial_digital_out_4_v2_create(IndustrialDigitalOut4V2 *industrial_digital_out_4_v2, const char *uid, IPConnection *ipcon) {
	...
	device_create(industrial_digital_out_4_v2, uid, ipcon_p, 2, 0, 1, INDUSTRIAL_DIGITAL_OUT_4_V2_DEVICE_IDENTIFIER);
	...
	device_p->response_expected[INDUSTRIAL_DIGITAL_OUT_4_V2_FUNCTION_WRITE_WIEGAND_DATA_LOW_LEVEL] = DEVICE_RESPONSE_EXPECTED_TRUE;
	device_p->response_expected[INDUSTRIAL_DIGITAL_OUT_4_V2_FUNCTION_GET_WIEGAND_STATE] = DEVICE_RESPONSE_EXPECTED_ALWAYS_TRUE;
	...
	device_p->callback_wrappers[INDUSTRIAL_DIGITAL_OUT_4_V2_CALLBACK_WIEGAND_DONE] = industrial_digital_out_4_v2_callback_wrapper_wiegand_done;
...
int industrial_digital_out_4_v2_write_wiegand_data_low_level(IndustrialDigitalOut4V2 *industrial_digital_out_4_v2, uint16_t data_length, bool data_data[256]) {
	DevicePrivate *device_p = industrial_digital_out_4_v2->p;
	WriteWiegandDataLowLevel_Request request;
	int ret;
	int i;
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_DIGITAL_OUT_4_V2_FUNCTION_WRITE_WIEGAND_DATA_LOW_LEVEL, device_p->ipcon_p, device_p);
	...
	memset(request.data_data, 0, 32); for (i = 0; i < 256; i++) request.data_data[i / 8] |= (data_data[i] ? 1 : 0) << (i % 8);
...
int industrial_digital_out_4_v2_get_wiegand_state(IndustrialDigitalOut4V2 *industrial_digital_out_4_v2, uint8_t *ret_wiegand_state) {
	DevicePrivate *device_p = industrial_digital_out_4_v2->p;
	GetWiegandState_Request request;
	GetWiegandState_Response response;
	int ret;
	...
	ret = packet_header_create(&request.header, sizeof(request), INDUSTRIAL_DIGITAL_OUT_4_V2_FUNCTION_GET_WIEGAND_STATE, device_p->ipcon_p, device_p);
	...
	*ret_wiegand_state = response.wiegand_state;
...
int industrial_digital_out_4_v2_write_wiegand_data(IndustrialDigitalOut4V2 *industrial_digital_out_4_v2, bool *data, uint16_t data_length) {
	...
	memcpy(data_data, data, sizeof(bool) * data_length);
	memset(&data_data[data_length], 0, sizeof(bool) * (256 - data_length));

	return industrial_digital_out_4_v2_write_wiegand_data_low_level(industrial_digital_out_4_v2, data_length, data_data);
}
